...
module.exports = class Persistent {
  ...
  unannounce (target, publicKey) {
    ...
    const hash = crypto.hex2buf(crypto.createHash(publicKey))

    if (b4a.equals(hash, target)) this.dht._router.delete(k)
  ...
  onunannounce (req) {
    ...
    //   return
    // }
    if(!crypto.verify(signable, signature, peer.publicKey)) return
  ...
  _onrefresh (token, req) {
    ...
    const activeRefresh = b4a.toString(hash, 'hex')

    const r = this.refreshes.get(activeRefresh)
    if (!r) return

    const { announceSelf, k, record } = r
    const publicKey = record.subarray(0, 33)
  ...
  onannounce (req) {
    ...
    const hash = crypto.hex2buf(crypto.createHash(peer.publicKey))

    const k = b4a.toString(req.target, 'hex')
    const announceSelf = b4a.equals(hash, req.target)
  ...
  onimmutableput (req) {
    ...
    const hash = crypto.hex2buf(crypto.createHash(req.value))
    if (!b4a.equals(hash, req.target)) return
  ...
  static signMutable (seq, value, secretKey) {
    ...
    // const signable = b4a.allocUnsafe(32)
    ...
    const hmac = crypto.hmac(c.encode(m.mutableSignable, { seq, value }), NS.MUTABLE_PUT)
    const signature = crypto.sign(hmac, secretKey)
  ...
  static signAnnounce (target, token, id, ann, secretKey) {
    ...
    const signature = crypto.sign(annSignable(target, token, id, ann, NS.ANNOUNCE), secretKey)
    return signature
  }
  ...
  static signUnannounce (target, token, id, ann, secretKey) {
    ...
    const signature = crypto.sign(annSignable(target, token, id, ann, NS.UNANNOUNCE), secretKey)
    return signature
  }
}
...
function verifyMutable (signature, seq, value, publicKey) {
  ...
  const hmac = crypto.hmac(c.encode(m.mutableSignable, { seq, value }), NS.MUTABLE_PUT)
  return crypto.verify(hmac, signature, publicKey)
}
...
function annSignable (target, token, id, ann, ns) {
  ...
  const hash = crypto.hmac(b4a.concat([
    target,
    id,
    token,
    c.encode(m.peer, ann.peer), // note that this is the partial encoding of the announce message so we could just use that for perf

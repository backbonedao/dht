...
class Pair {
  ...
  _onutpconnection (rawSocket, data, ended, from) {
    ...
    if (from) {
      if (this._sockets.addPool(rpub, utp, from)) {
        utp.on('message', (buf, rinfo) => this._sockets.dht.onmessage(utp, buf, rinfo))
      }
    }

    rawSocket.on('error', () => this._sockets.gcPool(rpub, true))
    rawSocket.on('close', () => this._sockets.gcPool(rpub, false))
  ...
  connectUTP (addr, socket) {
    ...
    if (autoClose) {
      rawSocket.on('close', () => socket.close())
    }
  }
...
module.exports = class SocketPairer {
  ...
  addPool (publicKey, socket, address) {
    const id = crypto.createHash(publicKey)
  ...
  gcPool (publicKey, now) {
    const id = crypto.createHash(publicKey)
    const pool = this._socketPool.get(id)

    if (!pool || pool.socket.connections.length > 0) return

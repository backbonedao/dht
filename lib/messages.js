...
const peer = exports.peer = {
  ...
  encode (state, m) {
    c.fixed33.encode(state, m.publicKey)
    ipv4Array.encode(state, m.relayAddresses)
  },
  decode (state) {
    return {
      publicKey: c.fixed33.decode(state),
...
exports.announce = {
  ...
  encode (state, m) {
    ...
    if (m.signature) c.fixed65.encode(state, m.signature)
  },
  decode (state) {
    const flags = c.uint.decode(state)
    ...
    return {
      ...
      signature: (flags & 4) !== 0 ? c.fixed65.decode(state) : null
    }
  }
}
...
exports.mutablePutRequest = {
  preencode (state, m) {
    c.fixed33.preencode(state, m.publicKey)
    c.uint.preencode(state, m.seq)
    c.buffer.preencode(state, m.value)
    c.fixed65.preencode(state, m.signature)
  },
  encode (state, m) {
    c.fixed33.encode(state, m.publicKey)
    c.uint.encode(state, m.seq)
    c.buffer.encode(state, m.value)
    c.fixed65.encode(state, m.signature)
  },
  decode (state) {
    return {
      publicKey: c.fixed33.decode(state),
      seq: c.uint.decode(state),
      value: c.buffer.decode(state),
      signature: c.fixed65.decode(state)
    }
  }
}
